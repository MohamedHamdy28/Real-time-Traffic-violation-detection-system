...
class NearestNeighborDistanceMetric(object):
    ...
    def partial_fit(self, features, targets, active_targets):
        ...
        for feature, target in zip(features, targets):
            self.samples.setdefault(target, []).append(feature) # 设置字典默认值，1、如果 key 不存在，则添加默认值，并将特征feature添加到 value 中；2、若存在，则追加当前的 feature
            if self.budget is not None:
                self.samples[target] = self.samples[target][-self.budget:]   # 只取对应id的前 100 个特征向量内容，减少内存占用 
        self.samples = {k: self.samples[k] for k in active_targets}     # self.samples并剔除其中不活跃的。

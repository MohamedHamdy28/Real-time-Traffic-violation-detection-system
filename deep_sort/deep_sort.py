...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, ori_img):
        ...
        features = self._get_features(
            bbox_xywh, ori_img)   # reid网络，提取每个bbox的feature

        # [cx,cy,w,h] -> [x1,y1,w,h]
        bbox_tlwh = self._xywh_to_tlwh(bbox_xywh)
        bbox_x1y1x2y2 = [(self._xywh_to_xyxy(box)) for box in bbox_xywh]
        detections = [Detection(bbox_tlwh[i], conf, features[i], bbox_x1y1x2y2[i]) for i, conf in enumerate(
            confidences) if conf > self.min_confidence]  # 过滤掉置信度小于self.min_confidence的bbox，生成detections
        ...
        detections = [detections[i]
                      for i in indices]           # 筛选出有效的detection

        # update tracker        # ------------卡尔曼滤波预测阶段
        self.tracker.predict()          # 卡尔曼滤波估计值
        # 卡尔曼滤波观测值  + 精确值；  精确值 = A * 卡尔曼滤波估计值 + B * 卡尔曼滤波观测值
